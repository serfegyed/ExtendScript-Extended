...
function expressionType(tokens, index, variableTypes, nativeCatalog, polyfillCatalog) {
    ...
    if (token.type === "identifier" && variableTypes.hasOwnProperty(token.value)) {
        baseType = variableTypes[token.value];
    }
    if (token.type === "identifier" && nativeCatalog.types[token.value] &&
            !variableTypes.hasOwnProperty(token.value)) {
    ...
    for (index = 0; index < tokens.length; index++) {
        if (tokens[index].value === "var") {
            ...
            while (index < tokens.length) {
                if (tokens[index].type === "identifier") declared[tokens[index].value] = true;
                while (index < tokens.length && tokens[index].value !== "," && tokens[index].value !== ";") index++;
                if (!tokens[index] || tokens[index].value === ";") break;
                index++;
            }
        } else if (tokens[index].value === "function") {
            if (tokens[index + 1] && tokens[index + 1].type === "identifier") declared[tokens[index + 1].value] = true;
        }
    }
    return declared;
}
...
function analyze(source, nativeCatalog, polyfillCatalog) {
    ...
    var declarations = collectDeclarations(tokens);
    var variableTypes = {};
    ...
    function receiverTypeAt(tokenIndex) {
        ...
        if (receiver.type === "identifier" && variableTypes.hasOwnProperty(receiver.value)) {
            return variableTypes[receiver.value];
        }
        ...
        if (receiver.value === ")" && pairs[tokenIndex] !== undefined) {
            ...
            if (callee.type === "identifier" && tokens[openIndex - 2] && tokens[openIndex - 2].value === ".") {
                ...
                if (base && base.type === "identifier" && nativeCatalog.types[base.value] &&
                        !variableTypes.hasOwnProperty(base.value)) {
                    baseType = base.value;
                    mode = "static";
    ...
    for (index = 0; index < tokens.length; index++) {
        ...
        if ((token.value === "var" || (token.type === "identifier" && next && next.value === "=")) && next) {
            var nameToken = token.value === "var" ? next : token;
            var equalsIndex = token.value === "var" ? index + 2 : index + 1;
            if (tokens[equalsIndex] && tokens[equalsIndex].value === "=") {
                ...
                if (!inferred && tokens[equalsIndex + 1] && variableTypes[tokens[equalsIndex + 1].value]) {
                    inferred = variableTypes[tokens[equalsIndex + 1].value];
                }
                variableTypes[nameToken.value] = inferred || null;
            }
        }
        ...
        if (token.type === "identifier" && token.value === "console" && !declarations.console) {
        ...
        if (!receiverType) {
            ...
            } else if (token.type === "identifier" && tokens[index + 3] && tokens[index + 3].value === "(" && !declarations[token.value]) {
